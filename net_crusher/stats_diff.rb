...
valuelist2.keys.each  do |key|
  ...
  puts "#{fill(key, 15)}#{fill(sprintf("%.2f", avgdiff), 10)}#{fill(sprintf("%.2f%%", 100 - avgdiff_percent),10)}#{fill(sprintf("%.2f", percent90diff), 10)}#{fill(sprintf("%.2f%%", 100 -  percent90diff_percent), 10)}"
end
